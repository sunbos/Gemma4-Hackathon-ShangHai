...
代码中仍保留音频录制能力，主要用于实验；当前产品方向是 **手写优先**，正式演示建议不要把语音作为主路径。

## 产品定位

AshPage 是一个私密、离线、阅后即焚的情绪出口。它不应该被包装成医疗、心理治疗或诊断产品。更准确的体验是：用户把不想说出口的话写到一张“活纸页”上，然后某个人格在同一张纸上回应。
...
- 对评审来说，按上面的 Gradle 构建步骤加 `adb push model.litertlm` 即可完成部署。
