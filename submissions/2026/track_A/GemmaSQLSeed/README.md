...
# 指定后端
bash quickstart.sh --backend lm_studio    # LM Studio 本地（默认）
bash quickstart.sh --backend ollama        # Ollama 本地
bash quickstart.sh --backend google        # Google AI Studio 云端
```
...
# 指定后端
.\quickstart.ps1 -Backend lm_studio
.\quickstart.ps1 -Backend ollama
.\quickstart.ps1 -Backend google
```
...
├── quickstart.sh         # 一键启动入口（Linux/macOS）
├── quickstart.ps1        # 一键启动入口（Windows）
├── tests/                # 测试套件
└── docs/                 # 文档
```
