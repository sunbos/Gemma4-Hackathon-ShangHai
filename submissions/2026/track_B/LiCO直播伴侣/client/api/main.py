    ...
    async def get(self, path, include_body=True):
        ...
        try:
            ...
            if e.status_code != 404:
                raise
            # 只有“前端路由”(通常无扩展名)才回退到 index.html。
            # 对于 *.js / *.css 等静态资源，如果不存在就保持 404，
            # 否则会把 index.html 当作 JS 执行，出现 "Unexpected token '<'"。
            if '.' in path:
                raise
            await self._get_index(include_body)
...
def _is_local_request(handler: api.base.ApiHandler) -> bool:
...
class ShutdownHandler(api.base.ApiHandler):
    async def post(self):
        if not _is_local_request(self):
            raise tornado.web.HTTPError(403, 'shutdown is only allowed from localhost')
        if not services.app_lifecycle.request_shutdown():
            raise tornado.web.HTTPError(503, 'shutdown is not available')
        logger.info('Shutdown requested by client=%s', self.request.remote_ip)
