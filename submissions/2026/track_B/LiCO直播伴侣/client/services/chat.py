...
# 弹幕落盘目录
_DANMAKU_LOG_DIR = os.path.join(config.DATA_PATH, 'danmaku_logs')
...
def init():
    ...
    os.makedirs(_DANMAKU_LOG_DIR, exist_ok=True)
...
class ClientRoom:
    def __init__(self, room_key: RoomKey):
        ...
        self._recorder = _DanmakuRecorder(room_key)
    ...
    def send_cmd_data(self, cmd, data):
        self._recorder.record(cmd, data)
...
class _DanmakuRecorder:
    """把房间消息落盘成 jsonl（每行一个 JSON），文件名：blivechat_<timestamp>.json"""

    # 只记录这些“弹幕相关”的命令
    # 注意：这里不能在模块 import 阶段引用 api.chat.Command（会与 api.chat <-> services.chat 循环引用冲突）
    # Command 值见 api/chat.py::Command
    _RECORD_CMD_INTS = {2, 3, 4, 5, 6, 7}
    ...
    def __init__(self, room_key: RoomKey):
        ...
        ts = int(time.time())
        filename = f'blivechat_{ts}.json'
        self._path = os.path.join(_DANMAKU_LOG_DIR, filename)
        try:
            os.makedirs(_DANMAKU_LOG_DIR, exist_ok=True)
            logger.info('room=%s danmaku log dir: %s', room_key, _DANMAKU_LOG_DIR)
            ...
            logger.info('room=%s danmaku log started: %s', room_key, self._path)
        except Exception:  # noqa
            logger.exception('room=%s failed to open danmaku log: %s', room_key, self._path)
            self._fp = None
    ...
    def record(self, cmd, data):
        ...
        try:
            ...
            return
        if cmd_int not in self._RECORD_CMD_INTS:
            return
    ...
    def close(self):
        ...
        try:
            self._fp.flush()
            self._fp.close()
            logger.info('room=%s danmaku log closed: %s', self._room_key, self._path)
        except Exception:  # noqa
            logger.exception('room=%s failed to close danmaku log: %s', self._room_key, self._path)
        finally:
            self._fp = None
...
class LiveMsgHandler(blivedm.BaseHandler):
    ...
    def _on_gift(self, client: WebLiveClient, message: dm_web_models.GiftMessage):
        ...
        services.psych_agent.on_gift(room_id=client.room_id, total_coin=data['totalCoin'])
        services.plugin.broadcast_cmd_data(
    ...
    async def __on_buy_guard(client: WebLiveClient, message: dm_web_models.UserToastV2Message):
        ...
        services.psych_agent.on_member(room_id=client.room_id)
        services.plugin.broadcast_cmd_data(
    ...
    def _on_super_chat(self, client: WebLiveClient, message: dm_web_models.SuperChatMessage):
        ...
        services.psych_agent.on_super_chat(
            room_id=client.room_id,
            price=message.price,
            content=message.message,
        )
